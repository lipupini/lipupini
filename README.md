...
- When serving image and text files from cache on-request, go ahead and check `filemtime` with the source file to see if the cache file needs to be invalidated. Video and audio is already solved because they use symlinks.
